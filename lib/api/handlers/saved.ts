...
import { intParam, UUID_RE } from "./params"
...
export async function savedGET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(apiError("server_error", "saved list failed"), { status: 500 })
  }
}

export async function savedPOST(request: Request) {
  const userId = userIdFrom(request)
  if (!userId) return NextResponse.json(MISSING_USER, { status: 400 })
  ...
  if (!insightId || !UUID_RE.test(insightId)) {
    return NextResponse.json(
      apiError("bad_request", "body must be { insightId: <uuid> }"),
      { status: 400 },
    )
  }
  ...
  try {
    ...
    return NextResponse.json(apiError("server_error", "save failed"), { status: 500 })
  }
}

export async function savedDELETE(request: Request) {
  const userId = userIdFrom(request)
  if (!userId) return NextResponse.json(MISSING_USER, { status: 400 })

  const insightId = new URL(request.url).searchParams.get("insightId")
  if (!insightId || !UUID_RE.test(insightId)) {
    return NextResponse.json(apiError("bad_request", "insightId query parameter required"), { status: 400 })
  }
  ...
  try {
    ...
    return NextResponse.json(apiError("server_error", "unsave failed"), { status: 500 })
  }
}
