...
export async function insightShareGET(
  request: Request,
...
export async function insightSharePOST(
  request: Request,
  ctx: { params: Promise<{ id: string }> },
) {
  ...
  try {
    ...
    if (!image) {
      return NextResponse.json(
        apiError(
          "not_found",
          "insight not found, or its stored quote span no longer fits its document",
        ),
        { status: 404 },
      )
    }
    ...
    return NextResponse.json(apiError("server_error", "share image generation failed"), { status: 500 })
  }
}
