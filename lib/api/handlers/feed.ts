...
import { isConfidenceLabel, type ConfidenceLabel } from "@/lib/confidence/score"
...
import { userIdFrom } from "../identity"
import { apiError } from "../version"
import { boolParam, csv, dateParam, intParam } from "./params"
 ...
 *   ?limit=20            capped server-side at 50
 *   ?sort=recent|relevant|confidence|corroboration
 ...
 *   ?confidence=high,medium          only cards with one of these labels
 *   ?verifyYourself=true|false       only the nudge cards, or only the rest
 *   ?minCorroboration=2              only cards confirmed by N+ independent sources
 *   ?candidateId=<uuid>              one ballot line
 *   ?speakerId=<uuid>                one PERSON, across races and spellings
 *   ?sourceDiversityLimit=3          how many corroborating sources to inline
 ...
 *
 * Every card carries `presentationMode`. A client branches on that rather than
 * on `claimSupportConfidence`: the threshold lives on the server so it can be
 * moved without a client release, and so two surfaces cannot disagree about
 * where the line is.
 */
export async function feedGET(request: Request) {
  ...
  const confidenceLabels = csv(params.get("confidence")).filter(
    (c): c is ConfidenceLabel => isConfidenceLabel(c),
  )
  ...
  try {
    ...
    const page = await getInsightFeed({
      ...
      flags,
      confidenceLabels,
      verifyYourself: boolParam(params.get("verifyYourself")),
      minCorroboration: intParam(params.get("minCorroboration")),
      sourceDiversityLimit: intParam(params.get("sourceDiversityLimit")),
    ...
    return NextResponse.json(apiError("server_error", "feed query failed"), { status: 500 })
  }
}

/** Kept exported so a route can re-use the date parser in the same shape. */
export { dateParam }
